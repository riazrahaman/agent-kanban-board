...
- **`useVisitCount.ts`:** v2.9.1 anonymous visit counter for the About footer. `useVisitCount()` counts the first page-view per browser session (`sessionStorage` flag `kanban-visit-counted`) against the public `abacus.jasoncameron.dev` counter under its own namespace `agent-kanban.riazrahaman.com`, returning `{count, counted}` (null when offline/blocked — the UI simply omits the line). `formatVisitCount` renders `1 visit` / `1,234 visits`. Deliberately not per-visitor tracking. Guarded by `useVisitCount.test.mjs`.
