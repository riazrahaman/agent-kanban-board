...
test('BUILDING/IN_REVIEW/IN_TEST count as active regardless of assigned_agent', () => {
  ...
  assert.equal(computeSignalStats(tasks).active, 4)
})
...
test('BLOCKED is counted exactly, case-sensitively', () => {
  ...
  assert.equal(computeSignalStats(tasks).blocked, 2)
})

test('done counts every DONE task as a total, regardless of logs or timing', () => {
  // The tile is a total DONE count, not a rolling "done today" window: a task
  // finalized without agent_logs, or that crossed a day boundary, still counts.
  const tasks = [
    task({ id: 'done-no-logs', status: 'DONE', agent_logs: [] }),
    task({ id: 'done-no-updated', status: 'DONE' }),
    task({
      id: 'done-old-logs',
      status: 'DONE',
      agent_logs: [{ timestamp: '2020-01-01T00:00:00Z', agent_id: 'x', message: 'm' }],
      updated: '2020-01-01T00:00:00Z',
    }),
    task({ id: 'not-done', status: 'IN_TEST', agent_logs: [] }),
    task({ id: 'backlog', status: 'BACKLOG' }),
  ]
  assert.equal(computeSignalStats(tasks).done, 3)
})

test('done ignores non-DONE statuses even when their logs look recent', () => {
  const tasks = [
    task({
      id: 'a',
      status: 'BUILDING',
      agent_logs: [{ timestamp: '2026-09-11T02:57:00Z', agent_id: 'x', message: 'm' }],
      updated: '2026-09-11T02:57:00Z',
    }),
    task({ id: 'b', status: 'DONE' }),
  ]
  assert.equal(computeSignalStats(tasks).done, 1)
})
...
test('regression: the reported bug (all tiles stuck at 0) is fixed against realistic multi-task data', () => {
  ...
  const tasks = [
    task({ id: 'done-today-1', status: 'DONE', agent_logs: [{ timestamp: '2026-09-11T02:57:00Z', agent_id: 'x', message: 'm' }] }),
    task({ id: 'done-today-2', status: 'DONE', agent_logs: [{ timestamp: '2026-09-11T09:00:00Z', agent_id: 'y', message: 'm' }] }),
    task({ id: 'done-old', status: 'DONE', agent_logs: [] }),
    task({ id: 'backlog', status: 'BACKLOG' }),
  ]
  const stats = computeSignalStats(tasks)

  // None of these fixture tasks are BUILDING/IN_REVIEW/IN_TEST/BLOCKED.
  assert.equal(stats.active, 0)
  assert.equal(stats.blocked, 0)
  // Before the fix this was always 0 regardless of data; it must now reflect
  // the full total of DONE tasks.
