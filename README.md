...
2. **Git-Backed YAML Storage**:
   ...
   | `KANBAN_HOLDER_WRITE_RENEWS_ALL` | `1` | When on, a holder's log/PATCH/claim also re-arms all its other leases (scoped to the same project as the write). |
   | `KANBAN_MAX_CLAIMS_PER_AGENT` | *(unlimited)* | Cap on active claims one agent may hold at once. A fresh claim past the cap gets a `409 claim_limit`. Privileged roles (runner/system/human/admin) are exempt. |
...
| `POST` | `/api/tasks/:id/claim` | Claim task for agent (`agent_id` body, optional `lease_ms`) | Auth + Contention gated |
| `POST` | `/api/tasks/:id/heartbeat` | Renew the claimant's task lease (optional `lease_ms` re-windows it) | Auth required |
| `POST` | `/api/agents/:agent_id/heartbeat` | Renew EVERY active lease that agent holds in one call. `?project=` is required for a non-privileged (self) caller and narrows the sweep to that project; a privileged caller may omit it for an all-projects sweep. | Auth required |
...
| `POST` | `/api/tasks/next-claim` | Claim next available task (`?project=` scopes; role from header; optional `lease_ms`) | Auth required |
